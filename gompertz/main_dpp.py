...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output/gompertz/lngrowth_vit_gompertz',
    ...
    parser.add_argument('--resume', default=True, action='store_true', help='resume to checkpoint')
    parser.add_argument('--checkpoint-path', default='pretrained_models/gompertz_base.pth', help='resume from checkpoint') 
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (not args.distributed or (args.distributed and args.rank == 0)) and not epoch%1:
            print('Do validation')
            test_stats = evaluate(data_loader_val, model, device, criterion, num_class=args.nb_classes)
            metric_name = 'auc' 
            ...
            print(f'Best {metric_name}: {max_accuracy:.3f}')     
