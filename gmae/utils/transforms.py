...
class Compose(object):
    ...
    def __call__(self, **data_dict):
        ...
        data: shape of [D,H,W] or [H,W]
        centerd: shape of [N,6] or [N, 4]
        '''
...
class RandomPadCrop(object):
    ...
    def __call__(self, **data_dict):
        # pad to crop size
        image_shape = data_dict['data'].shape
        diff_dim = len(image_shape) - len(self.size)
        assert diff_dim>=0, 'Dim of image smaller than size'
        image_shape = image_shape[diff_dim:]
        if np.any([image_shape[dim] < ps for dim, ps in enumerate(self.size)]):
            ...
            pad_list = [[0,0]]*diff_dim+[list(i) for i in zip(pad_below, pad_above)]
            data_dict['data'] = np.pad(data_dict['data'], pad_list, mode='constant', constant_values=self.pad_value)
            if data_dict['mask'] is not None:
                data_dict['mask'] = np.pad(data_dict['mask'], pad_list, mode='constant', constant_values=0)
            if data_dict['centerd'] is not None:
        ...
        # random crop
        img = data_dict['data']
        for ii in range(diff_dim, len(img.shape)):
            diff = img.shape[ii]-self.size[ii-diff_dim]
            if diff>0:
                min_crop = random.randint(0, diff)
                max_crop = min_crop + self.size[ii-diff_dim]
                data_dict['data'] = np.take(data_dict['data'], indices=range(min_crop, max_crop), axis=ii)
                if data_dict['mask'] is not None:
                    data_dict['mask'] = np.take(data_dict['mask'], indices=range(min_crop, max_crop), axis=ii)
                if data_dict['centerd'] is not None:
...
class RandomZFlip(object):
    ...
    def __call__(self, **data_dict):
        if random.random() < self.prob:
            ...
            if data_dict['centerd'] is not None:
                data_dict['centerd'][:, ii] = data_dict['data'].shape[ii]-1 - data_dict['centerd'][:, ii]
        return data_dict


class RandomFlipAxis(object):
    def __init__(self, prob=0.5, axis=0, coord_start_axis=0):
        self.prob = prob
        self.axis = axis
        self.coord_start_axis=coord_start_axis
    ...
    def __call__(self, **data_dict):
        if random.random() < self.prob:
            ...
            if data_dict['mask'] is not None:
                data_dict['mask'] = np.flip(data_dict['mask'], ii)
            if data_dict['centerd'] is not None:
                cii = self.axis - self.coord_start_axis
                data_dict['centerd'][:, cii] = data_dict['data'].shape[ii]-1 - data_dict['centerd'][:, ii]
        return data_dict
...
class ZeroOut(object):
    ...
    def __call__(self, **data_dict):
        d,h,w = data_dict['data'].shape #size
...
class ToTensor(object):
    def __call__(self, **data_dict):
        ...
        '''
        data = np.expand_dims(data_dict['data'], 0)
        data_dict['data'] = torch.from_numpy(data.astype(np.float32))
